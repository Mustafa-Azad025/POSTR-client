...
function ChatInput() {
	const { selectedChat, setSendMessageData } = useChat();
	const { data: session } = useSession();
	const { email, name }: any = session?.user;
	const [load, setLoad] = useState("");
	const [showEmoji, setShowEmoji] = useState(false);
	const user = [selectedChat]?.[0]?.users?.filter((item: any) => {
	...
	const sendMessage = async () => {
		if (load && [selectedChat][0]._id) {
			try {
				const { data } = await axios.post(
					`http://localhost:5000/api/chatmessages/`,
					{
						message: load,
						...
						<div className="w-full ml-3 sm:ml-0">
							<div className="flex flex-1 w-full">
								...
								<button
									...
									disabled={load.trim() == "" || ![selectedChat][0]._id}
									onClick={() => sendMessage()}
								>
